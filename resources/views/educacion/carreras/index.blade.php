...
<div class="vue-carreras">
    @include('layouts.headerpage', array('titulo'=>'Carreras'))
        ...
        <div class="card">
            <div class="card-body">
                <div class="form-group form-group-search d-flex search-field">
                    <input type="text" class="form-control " v-model="filtro" name="carrera" placeholder="Busca un carrera" v-on:keyup.enter="getCarreras()">
                    <button class="btn btn-primary ml-3" @click.prevent="getCarreras()">Buscar</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="form-group">
        <a href=" {{ route('carreras.create') }}  " class="btn btn-success" style="margin: 10px 0; ">Agregar nueva carrera</a>
                    ...
                    <template>
                        <div class="tablavue tablavue-header">
                            <div class="item-row">
                                ...
                                <div class="item-col">Facultad</div>
                                <div class="item-col th-pointer" @click.prevent="toggleOrderBy('Ciclos')">Ciclos <i v-if="orderby.field == 'Ciclos'" class="mdi position-absolute ml-1" v-bind:class="{ 'mdi-sort-ascending': orderby.type, 'mdi-sort-descending': !orderby.type }"></i></div>
                        ...
                        <div class="tablavue">
                            <div class="item-row" v-for="(carrera, index) in carreras">
                                <div class="item-col">
                                    ...
                                    <span class="item-tag">@{{carrera.Codigo}}</span></div>
                                <div class="item-col">
                                    <span class="item-title">Nombre: </span>
                                    <span class="item-tag">@{{carrera.Nombre}}</span>
                                </div>
                                <div class="item-col">
                                    <span class="item-title">Facultad: </span>
                                    <span class="item-tag">@{{ (carrera.Facultad)? carrera.Facultad.Nombre:'-' }}</span>
                                </div>
                                <div class="item-col">
                                    <span class="item-title">Ciclos: </span>
                                    <span class="item-tag">@{{carrera.Ciclos}}</span>
                                </div>
                                <div class="item-col item-col-options white">
                                    <span class="item-tag">
                                        ...
                                        <div class="dropdown-menu">
                                            <a class="dropdown-item" :href="url+carrera.id+'/edit'">Editar</a>
                                            <a class="dropdown-item" @click.prevent="confirmDialog(carrera)">Eliminar</a>
                                        </div>
                                    </span>
                                </div>
                            </div>
