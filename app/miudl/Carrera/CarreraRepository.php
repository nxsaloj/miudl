...
class CarreraRepository extends BaseRepository implements CarreraRepositoryInterface
{
    public function getModel($params=[])
    {
        return new Carrera($params);
    }
    ...
    public function search(array $params = array(), $paginate = false, $per_page=25)
    {
        ...
        $data = $this->getModel()->whereNull($this->getModel()->getTable().'.Deleted_at')->join('TB_Facultad','TB_Facultad.id',$this->getModel()->getTable().'.Facultad_id');
        //Filtrar por like
        if($filtro)
        {
            $data = $data->where( function ( $query ) use ($filtro)
            {
                ...
                ->orWhere('TB_Facultad.Nombre','like','%'.$filtro.'%');
            });
        }
        ...
        $extrafields = ["TB_Facultad.Nombre as Facultad_Nombre"];
